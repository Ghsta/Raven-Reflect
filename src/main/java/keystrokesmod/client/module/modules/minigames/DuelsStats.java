...
public class DuelsStats extends Module {
   ...
   public void update() {
      ...
      for (ScorePlayerTeam team : Minecraft.getMinecraft().world.getScoreboard().getTeams()) {
         for (String playerName : team.getMembershipCollection()) {
            if (!queue.contains(playerName) && team.getColor().equals("§7§k") && !playerName.equalsIgnoreCase(Minecraft.getMinecraft().player.getDisplayNameString())) {
