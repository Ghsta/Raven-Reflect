...
import zen.relife.util.GlyphPage;
...
public class GlyphPageFontRenderer {
    ...
    private void doDraw(float f, GlyphPage glyphPage) {
        ...
        if (this.underlineStyle) {
            ...
            bufferBuilder.pos(this.posX + (float) l, this.posY + (float) glyphPage.getMaxFontHeight(), 0.0D).endVertex();
            bufferBuilder.pos(this.posX + f, this.posY + (float) glyphPage.getMaxFontHeight(), 0.0D).endVertex();
            bufferBuilder.pos(this.posX + f, this.posY + (float) glyphPage.getMaxFontHeight() - 1.0F, 0.0D).endVertex();
            bufferBuilder.pos(this.posX + (float) l, this.posY + (float) glyphPage.getMaxFontHeight() - 1.0F, 0.0D).endVertex();
            tessellator.draw();
            GlStateManager.enableTexture2D();
        }
