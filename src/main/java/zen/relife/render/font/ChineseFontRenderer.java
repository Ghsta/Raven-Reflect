...
public class ChineseFontRenderer extends CFont {
    ...
    public ChineseFontRenderer(final Font font, final boolean antiAlias, final boolean fractionalMetrics) {
    ...
    public ChineseFontRenderer(final Font font, final boolean antiAlias, final boolean fractionalMetrics, final boolean chinese, final double offsetX, final double offsetY) {
    ...
    public ChineseFontRenderer(final Font font, final boolean antiAlias, final boolean fractionalMetrics, final double offsetX, final double offsetY) {
        super(font, antiAlias, fractionalMetrics);
        this.colorCode = new int[32];
        this.colorcodeIdentifiers = "0123456789abcdefklmnor";
        this.boldChars = new CharData[256];
    ...
    public int getStringWidth(final String text) {
        ...
        for (int size = text.length(), i = 0; i < size; ++i) {
            ...
            if (character >= '\u0100') {
                try {
                    character = (char) (this.charmap.get(character) + '\u0100');
                } catch (Exception ex) {
                }
            }
    ...
    public float drawOutlineString(final String text, final double x, final double y, final int color, final int outlineColor) {
        final String outlinetext = StringUtils.removeFormattingCodes(text);
        this.drawString(outlinetext, x + 0.5, y, outlineColor, false);
        this.drawString(outlinetext, x - 0.5, y, outlineColor, false);
        this.drawString(outlinetext, x, y + 0.5, outlineColor, false);
        this.drawString(outlinetext, x, y - 0.5, outlineColor, false);
        return this.drawString(text, x, y, color, false);
    }
    ...
    public float drawOutlineCenteredString(final String text, final double x, final double y, final int color, final int onlineColor) {
        return this.drawOutlineString(text, x - this.getStringWidth(text) / 2, y, color, onlineColor);
    }
}
