...
public class AimAssist extends Module {
    ...
    private final IntegerSetting speed = new IntegerSetting("Speed", 40.0, 1.0, 100.0, 1);
    private final IntegerSetting compliment = new IntegerSetting("Compliment", 15.0, 1.0, 100.0, 1);
    private final IntegerSetting fov = new IntegerSetting("FOV", 80.0, 1.0, 360.0, 1);
    private final IntegerSetting distance = new IntegerSetting("Distance", 4.3, 1.0, 10.0, 1);
    private final EnableSetting clickAim = new EnableSetting("ClickAim", true);
    private final EnableSetting Break_Blocks = new EnableSetting("Break Blocks", true);
    private final EnableSetting players = new EnableSetting("players", true);
    private final EnableSetting animals = new EnableSetting("animals", false);
    private final EnableSetting mobs = new EnableSetting("mobs", false);
    private final EnableSetting invisibles = new EnableSetting("invisibles", false);
    ...
    public static boolean autoClickerClicking() {
        Module autoclicker = Relife.INSTANCE.getModuleManager().getModule("AutoClicker");
        if (autoclicker.getState()) {
            return AutoClicker.Left_Click.getEnable() && Mouse.isButtonDown(0);
        }
        return false;
    }

    public static double fovFromEntity(Entity en) {
        return ((double) (AimAssist.mc.player.rotationYaw - AimAssist.fovToEntity(en)) % 360.0 + 540.0) % 360.0 - 180.0;
    }

    public static float fovToEntity(Entity ent) {
        double x = ent.posX - AimAssist.mc.player.posX;
        double z = ent.posZ - AimAssist.mc.player.posZ;
        ...
        block9:
        {
            ...
            block8:
            {
                ...
                block7:
                {
                    ...
                    if (this.Break_Blocks.getEnable() && AimAssist.mc.objectMouseOver != null && (p = AimAssist.mc.objectMouseOver.getBlockPos()) != null && (bl = AimAssist.mc.world.getBlockState(p).getBlock()) != Blocks.AIR && !(bl instanceof BlockLiquid) && bl instanceof Block) {
                        return;
                    }
                    autoclicker = Relife.INSTANCE.getModuleManager().getModule("AutoClicker");
                    if (!this.clickAim.getEnable()) break block7;
                    if (autoClickerClicking()) break block8;
                }
                if ((!Mouse.isButtonDown(0) || autoclicker.state) && this.clickAim.getEnable()) break block9;
            }
            if ((en = this.getEnemy()) != null && ((n = AimAssist.fovFromEntity(en)) > 1.0 || n < -1.0)) {
                ...
                AimAssist.mc.player.rotationYaw += val;
            }
        }
    }
    ...
    public Entity getEnemy() {
        ...
        Iterator var2 = AimAssist.mc.world.loadedEntityList.iterator();
        do {
            if (var2.hasNext()) continue;
            return null;
        } while ((en = (Entity) var2.next()) == AimAssist.mc.player || en.isDead || !this.invisibles.getEnable() && en.isInvisible() || (double) AimAssist.mc.player.getDistance(en) > this.distance.getCurrent() || AimAssist.bot(en));
