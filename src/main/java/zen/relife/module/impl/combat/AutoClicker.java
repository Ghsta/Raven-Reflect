...
public class AutoClicker extends Module {
    ...
    private final IntegerSetting LeftMinCPS = new IntegerSetting("MinCPS", 9.0, 1.0, 15.0, 1);
    private final IntegerSetting LeftMaxCPS = new IntegerSetting("MaxCPS", 13.0, 1.0, 15.0, 1);
    private final EnableSetting Right_Click = new EnableSetting("Right Click", false);
    private final IntegerSetting RightMinCPS = new IntegerSetting("RightMinCPS", 12.0, 1.0, 30.0, 1);
    private final IntegerSetting RightMaxCPS = new IntegerSetting("RightMaxCPS", 15.0, 1.0, 30.0, 1);
    private final EnableSetting Break_Blocks = new EnableSetting("Break Blocks", true);
    private final IntegerSetting BreakDelay = new IntegerSetting("Break Delay", 25.0, 0.0, 1000.0, 1);
    private final EnableSetting Weapon_only = new EnableSetting("Weapon Only", false);
    private final EnableSetting noBlockSword = new EnableSetting("noBlockSword", false);
    private final EnableSetting onlyBlocks = new EnableSetting("onlyBlocks", false);
    private final IntegerSetting JitterLeft = new IntegerSetting("JitterLeft", 0.0, 0.0, 3.0, 1);
    private final IntegerSetting JitterRight = new IntegerSetting("JitterRight", 0.0, 0.0, 3.0, 1);
    private final IntegerSetting rightClickDelay = new IntegerSetting("rightClickDelay", 85.0, 0.0, 500.0, 1);
    ...
    @SubscribeEvent
    public void onUpdate(TickEvent.RenderTickEvent ev) {
    ...
    private void Click(TickEvent.RenderTickEvent er, TickEvent.PlayerTickEvent e) {
        ...
        if (Mouse.isButtonDown(0) && Left_Click.getEnable()) {
            if (this.breakBlock()) {
                return;
            }
            ...
            if (this.JitterLeft.getCurrent() > 0.0) {
                ...
                if (this.rand.nextBoolean()) {
                    entityPlayer = AutoClicker.mc.player;
        ...
        if (Mouse.isButtonDown(1) && this.Right_Click.getEnable()) {
    ...
    public boolean breakBlock() {
        ...
        if (this.Break_Blocks.getEnable() && AutoClicker.mc.objectMouseOver != null && (p = AutoClicker.mc.objectMouseOver.getBlockPos()) != null) {
            Block bl = AutoClicker.mc.world.getBlockState(p).getBlock();
    ...
    public boolean rightClickAllowed() {
        ItemStack item = AutoClicker.mc.player.getHeldItemMainhand();
        if (item != null) {
