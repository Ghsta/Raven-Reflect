...
public class InvCleaner
    ...
    private final IntegerSetting BlockCap = new IntegerSetting("MaxBlocks", 128.0, 0.0, 256.0, 1);
    private final EnableSetting Food = new EnableSetting("Food", false);
    private final EnableSetting Sort = new EnableSetting("Sort", false);
    private final EnableSetting Bow = new EnableSetting("Bow", false);
    private final EnableSetting Sword = new EnableSetting("Sword", true);
    private final EnableSetting UHC = new EnableSetting("UHC", true);
    private final IntegerSetting Delay = new IntegerSetting("Delay", 50.0, 0.0, 1000.0, 1);
    ...
    public void onPre(TickEvent.PlayerTickEvent e) {
        ...
        if (InvCleaner.mc.currentScreen == null || InvCleaner.mc.currentScreen instanceof GuiInventory || InvCleaner.mc.currentScreen instanceof GuiChat) {
            if (this.timer.delay((float) delay) && weaponSlot >= 36) {
                if (!InvCleaner.mc.player.inventoryContainer.getSlot(weaponSlot).getHasStack()) {
                    this.getBestWeapon(weaponSlot);
                } else if (!this.isBestWeapon(InvCleaner.mc.player.inventoryContainer.getSlot(weaponSlot).getStack())) {
                    this.getBestWeapon(weaponSlot);
                }
            }
            ...
            if (this.timer.delay((float) delay)) {
                for (int i = 9; i < 45; ++i) {
                    ItemStack is;
                    if (!InvCleaner.mc.player.inventoryContainer.getSlot(i).getHasStack() || !this.shouldDrop(is = InvCleaner.mc.player.inventoryContainer.getSlot(i).getStack(), i))
                        continue;
                    this.drop(i);
                    this.timer.reset();
                    if (delay > 0.0) break;
    ...
    public boolean shouldDrop(ItemStack stack, int slot) {
        ...
        if (slot == weaponSlot && this.isBestWeapon(InvCleaner.mc.player.inventoryContainer.getSlot(weaponSlot).getStack()) || slot == pickaxeSlot && this.isBestPickaxe(InvCleaner.mc.player.inventoryContainer.getSlot(pickaxeSlot).getStack()) && pickaxeSlot >= 0 || slot == axeSlot && this.isBestAxe(InvCleaner.mc.player.inventoryContainer.getSlot(axeSlot).getStack()) && axeSlot >= 0 || slot == shovelSlot && this.isBestShovel(InvCleaner.mc.player.inventoryContainer.getSlot(shovelSlot).getStack()) && shovelSlot >= 0) {
            return false;
        }
        if (stack.getItem() instanceof ItemArmor) {
            for (int type = 1; type < 5; ++type) {
                ItemStack is;
                if (InvCleaner.mc.player.inventoryContainer.getSlot(4 + type).getHasStack() && AutoArmor.isBestArmor(is = InvCleaner.mc.player.inventoryContainer.getSlot(4 + type).getStack(), type) || !AutoArmor.isBestArmor(stack, type))
                    continue;
                return false;
            }
        }
    ...
    private int getBlockCount() {
        int blockCount = 0;
        for (int i = 0; i < 45; ++i) {
            if (!InvCleaner.mc.player.inventoryContainer.getSlot(i).getHasStack()) continue;
            ItemStack is = InvCleaner.mc.player.inventoryContainer.getSlot(i).getStack();
            Item item = is.getItem();
            if (!(is.getItem() instanceof ItemBlock)) continue;
            blockCount += is.getMaxStackSize();
        }
        return blockCount;
    }
    ...
    private void getBestShovel(int slot) {
        for (int i = 9; i < 45; ++i) {
            ItemStack is;
            if (!InvCleaner.mc.player.inventoryContainer.getSlot(i).getHasStack() || !this.isBestShovel(is = InvCleaner.mc.player.inventoryContainer.getSlot(i).getStack()) || shovelSlot == i || this.isBestWeapon(is))
                continue;
            if (!InvCleaner.mc.player.inventoryContainer.getSlot(shovelSlot).getHasStack()) {
                this.swap(i, shovelSlot - 36);
                this.timer.reset();
                if (!(this.Delay.getCurrent() > 0.0)) continue;
                return;
            }
            if (this.isBestShovel(InvCleaner.mc.player.inventoryContainer.getSlot(shovelSlot).getStack())) continue;
            this.swap(i, shovelSlot - 36);
            this.timer.reset();
            if (!(this.Delay.getCurrent() > 0.0)) continue;
            return;
    ...
    private boolean isBestShovel(ItemStack stack) {
        ...
        for (int i = 9; i < 45; ++i) {
            ItemStack is;
            if (!InvCleaner.mc.player.inventoryContainer.getSlot(i).getHasStack() || !(this.getToolEffect(is = InvCleaner.mc.player.inventoryContainer.getSlot(i).getStack()) > value) || !(is.getItem() instanceof ItemSpade))
                continue;
            return false;
        }
        return true;
    ...
    private boolean isBestAxe(ItemStack stack) {
        ...
        for (int i = 9; i < 45; ++i) {
            ItemStack is;
            if (!InvCleaner.mc.player.inventoryContainer.getSlot(i).getHasStack() || !(this.getToolEffect(is = InvCleaner.mc.player.inventoryContainer.getSlot(i).getStack()) > value) || !(is.getItem() instanceof ItemAxe) || this.isBestWeapon(stack))
                continue;
            return false;
        }
        return true;
    ...
    private void getBestAxe(int slot) {
        for (int i = 9; i < 45; ++i) {
            ItemStack is;
            if (!InvCleaner.mc.player.inventoryContainer.getSlot(i).getHasStack() || !this.isBestAxe(is = InvCleaner.mc.player.inventoryContainer.getSlot(i).getStack()) || axeSlot == i || this.isBestWeapon(is))
                continue;
            if (!InvCleaner.mc.player.inventoryContainer.getSlot(axeSlot).getHasStack()) {
                this.swap(i, axeSlot - 36);
                this.timer.reset();
                if (!(this.Delay.getCurrent() > 0.0)) continue;
                return;
            }
            if (this.isBestAxe(InvCleaner.mc.player.inventoryContainer.getSlot(axeSlot).getStack())) continue;
            this.swap(i, axeSlot - 36);
            this.timer.reset();
            if (!(this.Delay.getCurrent() > 0.0)) continue;
            return;
    ...
    private void getBestPickaxe(int slot) {
        for (int i = 9; i < 45; ++i) {
            ItemStack is;
            if (!InvCleaner.mc.player.inventoryContainer.getSlot(i).getHasStack() || !this.isBestPickaxe(is = InvCleaner.mc.player.inventoryContainer.getSlot(i).getStack()) || pickaxeSlot == i || this.isBestWeapon(is))
                continue;
            if (!InvCleaner.mc.player.inventoryContainer.getSlot(pickaxeSlot).getHasStack()) {
                this.swap(i, pickaxeSlot - 36);
                this.timer.reset();
                if (!(this.Delay.getCurrent() > 0.0)) continue;
                return;
            }
            if (this.isBestPickaxe(InvCleaner.mc.player.inventoryContainer.getSlot(pickaxeSlot).getStack())) continue;
            this.swap(i, pickaxeSlot - 36);
            this.timer.reset();
            if (!(this.Delay.getCurrent() > 0.0)) continue;
            return;
    ...
    private boolean isBestPickaxe(ItemStack stack) {
        ...
        for (int i = 9; i < 45; ++i) {
            ItemStack is;
            if (!InvCleaner.mc.player.inventoryContainer.getSlot(i).getHasStack() || !(this.getToolEffect(is = InvCleaner.mc.player.inventoryContainer.getSlot(i).getStack()) > value) || !(is.getItem() instanceof ItemPickaxe))
                continue;
            return false;
        }
        return true;
    ...
    private float getToolEffect(ItemStack stack) {
        ...
        float value = 1.0f;
        if (item instanceof ItemPickaxe) {
            value = tool.getDestroySpeed(stack, (IBlockState) Blocks.STONE);
            if (name.toLowerCase().contains("gold")) {
                value -= 5.0f;
            }
        } else if (item instanceof ItemSpade) {
            value = tool.getDestroySpeed(stack, (IBlockState) Blocks.DIRT);
            if (name.toLowerCase().contains("gold")) {
                value -= 5.0f;
            }
        } else if (item instanceof ItemAxe) {
            value = tool.getDestroySpeed(stack, (IBlockState) Blocks.LOG);
            if (name.toLowerCase().contains("gold")) {
                value -= 5.0f;
            }
        } else {
        ...
        value = (float) ((double) value + (double) EnchantmentHelper.getEnchantmentLevel(protection, stack) * 0.0075);
        value = (float) ((double) value + (double) EnchantmentHelper.getEnchantmentLevel(protection, stack) / 100.0);
        return value;
    }
    ...
    private boolean isBadPotion(ItemStack stack) {
        if (stack != null && stack.getItem() instanceof ItemPotion) {
            ...
            return potion.getUnlocalizedNameInefficiently(stack) == null;
        }
        return false;
    }
    ...
    public boolean isBestWeapon(ItemStack stack) {
        ...
        for (int i = 9; i < 45; ++i) {
            ItemStack is;
            if (!InvCleaner.mc.player.inventoryContainer.getSlot(i).getHasStack() || !(this.getDamage(is = InvCleaner.mc.player.inventoryContainer.getSlot(i).getStack()) > damage) || !(is.getItem() instanceof ItemSword) && this.Sword.getEnable())
                continue;
            return false;
        }
    ...
    public void getBestWeapon(int slot) {
        for (int i = 9; i < 45; ++i) {
            ItemStack is;
            if (!InvCleaner.mc.player.inventoryContainer.getSlot(i).getHasStack() || !this.isBestWeapon(is = InvCleaner.mc.player.inventoryContainer.getSlot(i).getStack()) || !(this.getDamage(is) > 0.0f) || !(is.getItem() instanceof ItemSword) && this.Sword.getEnable())
                continue;
            this.swap(i, slot - 36);
            this.timer.reset();
            break;
