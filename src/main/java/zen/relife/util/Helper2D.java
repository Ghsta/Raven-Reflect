...
public class Helper2D {
     ...
     */
    public static void drawRoundedRectangle(int x, int y, int w, int h, int radius, int color, boolean blur, int index) {
        if (index == -1) {
    ...
    public static void drawPicture(int x, int y, int w, int h, int color, String location) {
        ...
        ResourceLocation resourceLocation = new ResourceLocation(Relife.NAME, location);
        Relife.INSTANCE.mc.getTextureManager().bindTexture(resourceLocation);
    ...
    public static void drawGradientRectangle(int x, int y, int w, int h, int startColor, int endColor) {
        ...
        bufferbuilder.pos(x + w, y, 0f).color(f1, f2, f3, f).endVertex();
        bufferbuilder.pos(x, y, 0f).color(f1, f2, f3, f).endVertex();
        bufferbuilder.pos(x, y + h, 0f).color(f5, f6, f7, f4).endVertex();
        bufferbuilder.pos(x + w, y + h, 0f).color(f5, f6, f7, f4).endVertex();
        tessellator.draw();
    ...
    public static void drawHorizontalGradientRectangle(int x, int y, int w, int h, int startColor, int endColor) {
        ...
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_COLOR);
        bufferbuilder.pos(x, y, 0f).color(f1, f2, f3, f).endVertex();
        bufferbuilder.pos(x, y + h, 0f).color(f1, f2, f3, f).endVertex();
        bufferbuilder.pos(x + w, y + h, 0f).color(f5, f6, f7, f4).endVertex();
        bufferbuilder.pos(x + w, y, 0f).color(f5, f6, f7, f4).endVertex();
        tessellator.draw();
        GlStateManager.shadeModel(7424);
        GlStateManager.disableBlend();
        GlStateManager.enableAlpha();
